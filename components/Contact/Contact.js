...
export default function Contact({ content, settings }) {
  ...
  return (
    <section id="contact" className="relative py-24 overflow-hidden">
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-violet-500/5 via-transparent to-pink-500/5" aria-hidden="true" />
      <div className="absolute top-0 right-0 w-96 h-96 bg-violet-500/10 rounded-full blur-3xl -z-10" aria-hidden="true" />
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-pink-500/10 rounded-full blur-3xl -z-10" aria-hidden="true" />
      ...
      <div className="section-wrapper relative z-10">
        ...
        >
          ...
          <h2 className="text-gray-100">{sectionTitle || 'Get in Touch'}</h2>
          {sectionSubtitle && (
          ...
          {responseTime && (
            <p className="text-sm text-violet-400/80 font-mono mt-2">
              {responseTime}
            </p>
          )}
        </motion.div>
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12 items-start">
              ...
              {socialLinks.map(({ icon: Icon, label, value, href }) =>
                href ? (
                  <motion.a
                    ...
                    rel="noreferrer"
                    whileHover={{ x: 4 }}
                    className="group glass-card flex items-center gap-4 p-5 transition-all duration-300 cursor-pointer hover:border-violet-500/50 hover:bg-violet-500/5"
                  >
          ...
          <motion.div
            ...
            className={socialLinks.length > 0 ? 'lg:col-span-3 order-1 lg:order-2' : 'lg:col-span-5 max-w-2xl mx-auto'}
          >
