...
function SettingsContent() {
  ...
  useEffect(() => {
    api.get('/api/settings').then(({ data }) => { if (data.settings) setSettings(data.settings) }).catch(() => {})
  }, [])
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2"><label className={LABEL_CLS}>Address</label><input value={settings.contactInfo?.address || ''} onChange={e => set('contactInfo.address', e.target.value)} placeholder="City, Country" className={INPUT_CLS} /></div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div><label className={LABEL_CLS}>Currently Building</label><input value={settings.widgets?.currentlyBuilding || ''} onChange={e => set('widgets.currentlyBuilding', e.target.value)} placeholder="RAG chatbot…" className={INPUT_CLS} /></div>
            <div><label className={LABEL_CLS}>Currently Reading</label><input value={settings.widgets?.currentlyReading || ''} onChange={e => set('widgets.currentlyReading', e.target.value)} placeholder="Reading…" className={INPUT_CLS} /></div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div><label className={LABEL_CLS}>Meta Title</label><input maxLength={60} value={settings.seo?.metaTitle || ''} onChange={e => set('seo.metaTitle', e.target.value)} placeholder="Your Name | Portfolio" className={INPUT_CLS} /></div>
          <div><label className={LABEL_CLS}>Meta Description</label><textarea maxLength={160} rows={2} value={settings.seo?.metaDescription || ''} onChange={e => set('seo.metaDescription', e.target.value)} placeholder="Personal portfolio…" className={`${INPUT_CLS} resize-none`} /></div>
          <div><label className={LABEL_CLS}>Keywords</label><input value={(settings.seo?.keywords || []).join(', ')} onChange={e => set('seo.keywords', e.target.value.split(',').map(k => k.trim()).filter(Boolean))} placeholder="portfolio, developer" className={INPUT_CLS} /></div>
        </motion.div>
