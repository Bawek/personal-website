...
function SyncContent() {
  ...
  const syncGitHub = async () => {
    ...
    try {
      const { data } = await api.post('/api/sync/github/sync', { username: gh.username }, { headers: headers() })
  ...
  const saveSettings = async () => {
    setLoading(true); setMsg(null)
    try {
      await api.post('/api/sync/settings', { autoSyncGithub: gh.autoSync, autoPostLinkedin: li.autoPost, githubUsername: gh.username, linkedinEnabled: li.enabled }, { headers: headers() })
