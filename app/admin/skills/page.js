...
function SkillsContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) await api.put(`/api/skills/${editing.slug}`, submitData, { headers: headers() })
      else         await api.post('/api/skills', submitData, { headers: headers() })
  ...
  const handleDelete = async (slug) => {
    ...
    try { await api.delete(`/api/skills/${slug}`, { headers: headers() }); await fetchSkills() }
