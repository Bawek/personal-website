    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-white/10">
        ...
        >
          Inbox
          {unread > 0 && (
            <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-violet-500/20 text-violet-300 border border-violet-500/30">
              {unread}
            </span>
          )}
          {activeTab === 'messages' && (
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          {editingContact ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea rows={2} value={contactForm.hero.subtitle} onChange={e => setContactForm({ ...contactForm, hero: { ...contactForm.hero, subtitle: e.target.value } })}
                      placeholder="Have a project in mind or just want to say hi?" className={`${INPUT_CLS} resize-none`} />
                  </div>
                </div>
              </div>
                        ...
                        <div key={link.id} className="flex gap-2 items-end">
                          <div className="flex-1 space-y-2">
                            <input value={link.platform} onChange={e => handleUpdateSocialLink(link.id, 'platform', e.target.value)}
                              placeholder="Platform" className={`${INPUT_CLS} text-xs`} />
              ...
              <div className="pb-6">
                ...
                <div>
                  ...
                  <input value={contactForm.footer.text} onChange={e => setContactForm({ ...contactForm, footer: { ...contactForm.footer, text: e.target.value } })}
                    placeholder="© 2026. All rights reserved." className={INPUT_CLS} />
                </div>
              </div>
            ...
            <div className="text-center py-8 text-gray-500">
              <p className="text-sm">Click "Edit" to modify the contact section settings</p>
            </div>
          )}
        </motion.div>
      )}
