...
function ChatContent() {
  ...
  const loadConversation = async (convId) => {
    try {
      const { data } = await api.get(`/api/chat/${convId}`, { headers: headers() })
  ...
  const fetchConversations = async () => {
    try {
      ...
      const { data } = await api.get(`/api/chat/admin/conversations?${params}`, {
        headers: headers()
      })
  ...
  const fetchStats = async () => {
    try {
      const { data } = await api.get('/api/chat/admin/stats', { headers: headers() })
      setStats(data.stats)
    } catch (error) {
  ...
  const handleDeleteConversation = async (conversationId) => {
    ...
    try {
      await api.delete(`/api/chat/${conversationId}`, { headers: headers() })
      if (selectedConversation?._id === conversationId) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">Chat Management</h1>
        <p className="text-gray-500 text-sm">Manage customer conversations and support</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Open', value: stats.open, color: 'from-yellow-500 to-yellow-600' },
            { label: 'Resolved', value: stats.resolved, color: 'from-green-500 to-green-600' },
            { label: 'Unread', value: stats.unread, color: 'from-red-500 to-red-600' }
          ].map((stat, idx) => (
            <motion.div
              key={idx}
              ...
              conversations.map((conv) => (
                <motion.button
                  ...
                  className={`w-full text-left p-4 border-b border-white/5 hover:bg-white/5 transition ${
                    selectedConversation?._id === conv._id ? 'bg-white/5' : ''
                  }`}
                  whileHover={{ x: 4 }}
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white truncate text-sm">{conv.visitorName}</p>
                      <p className="text-xs text-gray-400 truncate">{conv.visitorEmail}</p>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded border whitespace-nowrap ml-2 ${getStatusColor(conv.status)}`}>
                      {conv.status}
                    </span>
                  </div>
                  <p className="text-xs text-gray-300 truncate mb-2">{conv.subject}</p>
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <span>{conv.messages?.length || 0} msgs</span>
                    <HiFlag className={`w-3 h-3 ${getPriorityColor(conv.priority)}`} />
                  </div>
                </motion.button>
              ))
            )}
        ...
        <div className="lg:col-span-2 bg-white/5 border border-white/10 rounded-2xl overflow-hidden flex flex-col">
          {selectedConversation ? (
            <>
              ...
              {/* Input */}
              <form onSubmit={handleSendMessage} className="border-t border-white/10 p-4 bg-white/3">
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={messageInput}
                  ...
                  <button
                    ...
                    className="bg-violet-600 hover:bg-violet-700 disabled:bg-gray-600 text-white p-2 rounded transition"
                  >
